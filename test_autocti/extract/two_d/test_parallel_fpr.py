...
def test__array_2d_list_from(parallel_array, parallel_masked_array):
    ...
    front_edge_list = extract.array_2d_list_from(array=parallel_array, pixels=(0, 1))
    assert (front_edge_list[0] == np.array([[1.0, 1.0, 1.0]])).all()

    front_edge_list = extract.array_2d_list_from(array=parallel_array, pixels=(2, 3))
    ...
    front_edge_list = extract.array_2d_list_from(array=parallel_array, pixels=(0, 3))
    assert (
        front_edge_list[0]
...
def test__binned_array_1d_from(parallel_array, parallel_masked_array):
    ...
    front_edge_line = extract.binned_array_1d_from(array=parallel_array, pixels=(0, 3))

    assert (front_edge_line == np.array([3.0, 4.0, 5.0])).all()

    extract = ac.Extract2DParallelFPR(region_list=[(1, 3, 0, 3), (5, 8, 0, 3)])

    front_edge_line = extract.binned_array_1d_from(array=parallel_array, pixels=(0, 2))
