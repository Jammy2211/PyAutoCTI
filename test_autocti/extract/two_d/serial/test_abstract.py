...
def test__median_list_from(serial_array, serial_masked_array):
    ...
    median_list = extract.median_list_from(array=serial_array, pixels=(0, 3))

    assert median_list == [1.0, 1.0, 1.0]

    # Extend pixels to extract [0.0, 1.0, 2.0, 3.0, 4.0]

    median_list = extract.median_list_from(array=serial_array, pixels=(0, 5))
    assert median_list == [2.0, 2.0, 2.0]

    extract = ac.Extract2DSerialFPR(region_list=[(0, 2, 0, 4), (0, 2, 6, 10)])

    median_list = extract.median_list_from(array=serial_array, pixels=(0, 4))
...
def test__median_list_from__pixels_from_end(serial_array, serial_masked_array):
    ...
    median_list = extract.median_list_from(array=serial_array, pixels_from_end=5)
    assert median_list == [2.0, 2.0, 2.0]
    ...
    median_list = extract.median_list_from(array=serial_array, pixels_from_end=4)
    assert median_list == [4.5, 4.5]

    median_list = extract.median_list_from(array=serial_masked_array, pixels_from_end=4)
...
def test__median_list_of_lists_from(serial_array, serial_masked_array):
    ...
    median_list_of_lists = extract.median_list_of_lists_from(
        array=serial_masked_array, pixels=(0, 4)
    )
