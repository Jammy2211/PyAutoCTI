...
def test_region_list_from__via_array_2d_list_from__pixels_from_end(
    ...
    overscan_list = extract.array_2d_list_from(array=parallel_array, pixels_from_end=1)
    assert (overscan_list[0] == np.array([[3.0, 3.0, 3.0]])).all()

    overscan_list = extract.array_2d_list_from(array=parallel_array, pixels_from_end=3)
    assert (
        overscan_list[0]
        == np.array([[1.0, 1.0, 1.0], [2.0, 2.0, 2.0], [3.0, 3.0, 3.0]])
    ).all()
