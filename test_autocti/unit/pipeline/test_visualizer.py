...
def set_config_path(plot_path):
    conf.instance.push(new_path=path.join(directory, "config"), output_path=plot_path)
...
class TestPhaseCIImagingVisualizer:
    def test__visualizes_imaging_using_configs(
        ...
        assert (
            path.join(plot_path, "image", "subplots", "subplot_ci_imaging.png")
            in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "ci_imaging", "image.png") in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "ci_imaging", "noise_map.png")
            not in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "ci_imaging", "signal_to_noise_map.png")
            not in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "ci_imaging", "ci_pre_cti.png")
            in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "ci_imaging", "cosmic_ray_map.png")
            in plot_patch.paths
        )
    ...
    def test___visualizes_fit_using_configs(
        ...
        assert (
            path.join(plot_path, "image", "subplots", "subplot_ci_fit.png")
            in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "fit_ci_imaging", "image.png")
            in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "fit_ci_imaging", "noise_map.png")
            not in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "fit_ci_imaging", "signal_to_noise_map.png")
            not in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "fit_ci_imaging", "ci_pre_cti.png")
            in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "fit_ci_imaging", "ci_post_cti.png")
            in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "fit_ci_imaging", "residual_map.png")
            not in plot_patch.paths
        )
        assert (
            path.join(
                plot_path, "image", "fit_ci_imaging", "normalized_residual_map.png"
            )
            in plot_patch.paths
        )
        assert (
            path.join(plot_path, "image", "fit_ci_imaging", "chi_squared_map.png")
            in plot_patch.paths
        )
