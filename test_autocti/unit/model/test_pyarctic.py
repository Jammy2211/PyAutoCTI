...
class TestArcticAddCTI:
    ...
    class TestChargeInjectionMode:
        def test__charge_injection_parameters_on__makes_trails_longer(self):
            ...
            assert (
                image_difference[2, :] < 0.0
            ).all()  # With charge injection mode on, more transfers, so more charge lost
            assert (
                image_difference[3:-1, :] > 0.0
            ).all()  # And therefore, trails are brighter
