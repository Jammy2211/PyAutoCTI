...
def test__no_instrumental_effects_input__only_cti_simulated(clocker_1d, traps_x2, ccd):
    ...
    dataset = simulator.via_layout_from(layout=layout, clocker=clocker_1d, cti=cti)

    assert dataset.data == pytest.approx(np.array([9.43, 9.43, 9.43, 9.43, 9.43]), 1e-1)
...
def test__via_pre_cti_data(clocker_1d, traps_x2, ccd):
    layout = ac.Layout1D(shape_1d=(5,), region_list=[(0, 5)])
...
def test__via_post_cti_data(clocker_1d, traps_x2, ccd):
    layout = ac.Layout1D(shape_1d=(5,), region_list=[(0, 5)])
    simulator = ac.SimulatorDataset1D(
        pixel_scales=1.0,
        norm=10.0,
