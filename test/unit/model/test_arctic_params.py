...
class TestParams:
    def test__1_species__sets_values_correctly(self):
        ...
        parameters = ac.ArcticParams(parallel_species=parallel_1_species)

        assert type(parameters) == ac.ArcticParams
        assert type(parameters.parallel_species) == ac.Species

        assert parameters.parallel_species.trap_density == 0.1
        assert parameters.parallel_species.trap_lifetime == 1.0

    def test__parallel_x1__serial_x1_species___sets_value_correctly(self):
    ...
    def test__delta_ellpiticity_of_species(self):
        ...
        species = ac.Species(trap_density=0.5, trap_lifetime=2.0)
    ...
    def test__delta_ellipticity_of_arctic_params(self):
        ...
        parallel_1_species = ac.Species(trap_density=0.1, trap_lifetime=4.0)
        parallel_2_species = ac.Species(trap_density=0.1, trap_lifetime=4.0)
        serial_1_species = ac.Species(trap_density=0.2, trap_lifetime=2.0)
        serial_2_species = ac.Species(trap_density=0.7, trap_lifetime=7.0)
        ...
        parameters = ac.ArcticParams(serial_species=[serial_1_species])
...
class TestParallelDensityVary:
    def test_1_species__density_01__1000_column_pixels__1_row_pixel_so_100_traps__posison_density_near_01(
        ...
        parallel_vary = ac.Species.poisson_species(
            species=list(
                map(
                    lambda density: ac.Species(trap_density=density, trap_lifetime=1.0),
                    (0.1,),
                )
            ),
            shape=(1000, 1),
