...
SUB_PATH_LENGTH = 10
AUTOLENS_FOLDER = ".autocti"

logger = logging.getLogger(__file__)

home = expanduser("~")
autocti_dir = "{}/{}".format(home, AUTOLENS_FOLDER)

try:
    os.mkdir(autocti_dir)
...
def path_for(path):
    """
    Generate a path in the ~/.autocti directory by taking the provided path, base64 encoding it and extracting the
    ...
    return "{}/al_{}".format(autocti_dir, (encoded_string[:start] + encoded_string[-end:]).replace("-", ""))


def make_linked_folder(sym_path):
    """
    Create a folder in the ~/.autocti directory and create a sym link to it at the provided path.
