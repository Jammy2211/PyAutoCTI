...
parallel_species = arctic_params.Species(trap_density=3.0, trap_lifetime=1.0)
parallel_ccd = arctic_params.CCD(well_notch_depth=1.0e-4, well_fill_beta=0.58, well_fill_gamma=0.0, well_fill_alpha=1.0)
serial_species = arctic_params.Species(trap_density=80.0, trap_lifetime=1.0)
...
ci_datas = list(map(lambda ci_pre_cti, ci_pattern, cosmic_ray_image:
                    ci_data.simulate(ci_pre_cti=ci_pre_cti, frame_geometry=frame_geometry,
                                     ...
                                     read_noise=4.0, cosmic_ray_image=cosmic_ray_image, noise_seed=1),
...
ci_data_plotters.plot_image(ci_data=ci_datas[0], cmap='cool',
                            norm='symmetric_log', linthresh=400.0, linscale=2.0,
                            title='Cosmic Ray Charge Injection Image')
...
ci_data_plotters.plot_image(ci_data=ci_datas[0], cmap='cool',
                            norm='symmetric_log', linthresh=400.0, linscale=2.0,
                            title='Cosmic Ray Charge Injection Image',
                            output_path=ci_plot_path, output_filename='cosmic_rays', output_format='png')