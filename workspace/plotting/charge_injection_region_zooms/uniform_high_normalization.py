...
parallel_species = arctic_params.Species(trap_density=2.0, trap_lifetime=1.0)
parallel_ccd = arctic_params.CCD(well_notch_depth=1.0e-4, well_fill_beta=0.58, well_fill_gamma=0.0, well_fill_alpha=1.0)
serial_species = arctic_params.Species(trap_density=20.0, trap_lifetime=4.0)
...
ci_data_plotters.plot_ci_pre_cti(ci_data=ci_datas[0], cmap='cool')
ci_data_plotters.plot_image(ci_data=ci_datas[0], cmap='cool',
                            norm='symmetric_log', linthresh=84000.0, linscale=0.001,
                            cb_tick_values=[min_value, 84000.0, 84250.0, 84500.0, max_value],
                            cb_tick_labels=['{0:.1f}'.format(min_value), '84000.0', '84250.0', '84500.0', '84700.0'])

ci_data_plotters.plot_image(ci_data=ci_datas[0], cmap='cool', ylabelsize=0,
                            norm='symmetric_log', linthresh=1.0, linscale=0.00,
                            cb_tick_values=[min_value, 0.0, 10.0, 100.0, 1000.0, 10000.0, max_value],
                            cb_tick_labels=['{0:.1f}'.format(min_value), '0.0', '10.0', '100.0', '1000.0', '10000.0',
                                          '84700.0'])

# Now, lets output this simulated ccd-data to the data folder. First, we'll get a relative path.
path = '{}/'.format(os.path.dirname(os.path.realpath(__file__)))

ci_plot_path = path + '/plots/uniform_high_normalization/'

if not os.path.exists(ci_plot_path):
    os.makedirs(ci_plot_path)

ci_data_plotters.plot_ci_pre_cti(ci_data=ci_datas[0], cmap='cool', output_path=ci_plot_path,
                                 output_filename='ci_pre_cti', output_format='png')
ci_data_plotters.plot_image(ci_data=ci_datas[0], cmap='cool', output_path=ci_plot_path,
                            norm='symmetric_log', linthresh=84000.0, linscale=0.001,
                            cb_tick_values=[min_value, 84000.0, 84250.0, 84500.0, max_value],
                            cb_tick_labels=['{0:.1f}'.format(min_value), '84000.0', '84250.0', '84500.0', '84700.0'],
                            output_filename='ci_image', output_format='png')
...
ci_data_plotters.plot_image(ci_data=ci_datas[0], cmap='cool', ylabelsize=0,
                            ...
                            cb_tick_labels=['{0:.1f}'.format(min_value), '0.0', '10.0', '100.0', '1000.0', '10000.0',
                                            '84700.0'],
                            output_path=ci_plot_path, output_filename='ci_image_log', output_format='png')