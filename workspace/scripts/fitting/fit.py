...
ci_data_name = 'ci_x8_images_non_uniform_high_res' # Charge injection data consisting of 2 images with uniform injections.
ci_data_model = 'serial_x3_species_model_4_1' # Shows the data was creating using a serial CTI model with one species.
...
ci_regions = [(0, 330, 51, shape[1]-20), (330, 2316, 51, shape[1]-20)]
...
serial_species = arctic_params.Species(trap_density=0.508205235667156030E+00, trap_lifetime=0.550359503069515554E+01)
serial_ccd = arctic_params.CCD(well_fill_beta=0.568999288845809303E+00, well_notch_depth=-0.614727709673752543E+01,
                               well_fill_alpha=1.0, well_fill_gamma=0.0)
cti_params = arctic_params.ArcticParams(serial_species=[serial_species], serial_ccd=serial_ccd)

mask_function=msk.Mask.empty_for_shape
masks = list(map(lambda data: mask_function(shape=data.image.shape), datas))
phase = ph.SerialPhase(rows=(0, 10))
